...
function InputField({
  ...
  return (
    <div className={`${loading ? "animate-pulse" : ""} ${className}`}>
      <label
        className="block text-primary-green-400 phone:text-caption-3 screen:text-caption-3 screen:leading-caption-2 desktop:text-caption-3 twok:text-caption-2 desktop:font-semibold twok:font-bold mb-2"
        for={label}
      >
        {label} *
      </label>
      <div className="relative">
        ...
        {value && !loading && (
          <img
            ...
            className="w-4 bg-success-color-50 tablet:w-5 desktop:w-6  pointer-events-none absolute top-1/2 transform -translate-y-1/2 right-3"
          />
        )}
      </div>
    </div>
