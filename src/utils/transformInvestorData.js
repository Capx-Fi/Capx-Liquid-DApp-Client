...
export const transformInvestorData = async(
...
) => 
    {
        ...
        if(projectOverviewData.length > 0){
            for(let projectDisplayID = 0; projectDisplayID < projectOverviewData.length; projectDisplayID++){
                let projectName = projectOverviewData[projectDisplayID].projectName;
                let projectOwnerAddress = projectOverviewData[projectDisplayID].projectOwnerAddress;
                let projectTokenAddress = projectOverviewData[projectDisplayID].projectTokenAddress
                let projectTokenTicker = projectOverviewData[projectDisplayID].projectTokenTicker;
                let projectTokenDecimal = Number(projectOverviewData[projectDisplayID].projectTokenDecimal);
                                    ...
                                    .map((derivative, _index) => 
                                        derivative.holders.map((holder, index) => {
                                            ...
                                            let numOfTokens = new BigNumber(holder?.tokenAmount)
                                                .dividedBy(
                                                    Math.pow(10, projectTokenDecimal)
                                                )
                                                .toNumber();
                                            return {
                                                date: date,
                ...
                if(
                    ...
                    projects = [...projects, ...currentVestedProject, ...currentWrappedProject];
                } else {
                    if (
                        currentWrappedProject &&
                        Object.keys(currentWrappedProject)?.length > 0
